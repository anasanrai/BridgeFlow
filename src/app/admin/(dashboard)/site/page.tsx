    ...
    if (!config) return (
        <div className="text-center py-16 glass rounded-xl border border-dashed border-white/10">
            ...
            <button onClick={async () => {
                ...
                load();
            }} className="px-4 py-2 bg-brand-coral text-neutral-950 font-bold text-neutral-950 font-semibold rounded-lg text-sm">
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-display font-bold text-white flex items-center gap-2"><Globe className="w-6 h-6 text-brand-coral" />Site Configuration</h1>
            ...
            <div className="space-y-6">
                ...
                <div className="glass rounded-xl p-6 card-glow">
                        ...
                        {[
                            ...
                            <div key={field.key}>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 bg-neutral-900/80 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-gold-400/50"
                                />
                            </div>
                        ))}
                    </div>
                    <div className="mt-4">
                        ...
                        <textarea value={config.description || ""} onChange={e => setConfig({ ...config, description: e.target.value })} rows={2}
                            className="w-full px-4 py-2.5 bg-neutral-900/80 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-gold-400/50 resize-none" />
                    </div>
                </div>
                ...
                <div className="glass rounded-xl p-6 card-glow">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={addNavLink} className="flex items-center gap-1 text-xs text-brand-coral hover:text-gold-300"><Plus className="w-3 h-3" />Add</button>
                    </div>
                    <div className="space-y-2">
                        {(config.nav_links || []).map((link, i) => (
                            <div key={i} className="flex gap-3 items-center">
                                <input value={link.label} onChange={e => updateNavLink(i, "label", e.target.value)} className="flex-1 px-3 py-2 bg-neutral-900/80 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-gold-400/50" placeholder="Label" />
                                <input value={link.href} onChange={e => updateNavLink(i, "href", e.target.value)} className="flex-1 px-3 py-2 bg-neutral-900/80 border border-white/10 rounded-lg text-white text-sm font-mono focus:outline-none focus:border-gold-400/50" placeholder="/path" />
                ...
                <div className="glass rounded-xl p-6 card-glow">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={addSocial} className="flex items-center gap-1 text-xs text-brand-coral hover:text-gold-300"><Plus className="w-3 h-3" />Add</button>
                    </div>
                    <div className="space-y-2">
                        {(config.social_links || []).map((link, i) => (
                            <div key={i} className="flex gap-3 items-center">
                                <input value={link.platform} onChange={e => updateSocial(i, "platform", e.target.value)} className="w-32 px-3 py-2 bg-neutral-900/80 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-gold-400/50" placeholder="Twitter" />
                                <input value={link.href} onChange={e => updateSocial(i, "href", e.target.value)} className="flex-1 px-3 py-2 bg-neutral-900/80 border border-white/10 rounded-lg text-white text-sm font-mono focus:outline-none focus:border-gold-400/50" placeholder="https://..." />
